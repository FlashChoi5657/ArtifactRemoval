...
from utils import Custom_Dataset, save_checkpoint, CT_Dataset, CT_testset
...
def test(config, workdir='/nas/users/minhyeok/CMH/sde'):
    ...
    ckpt_filename = '/nas/users/minhyeok/CMH/sde/result/11_22/size128_M_ckpt_last.pth'
    ...
    # test_set = CT_Dataset(config.data.root, transform=transform, train=False)
    test_set = CT_testset(transform=transform, train=False)
    test_dl = DataLoader(dataset=test_set, batch_size=config.eval.batch_size, drop_last=True)
    ...
    if solver == 'mcg': 
        ...
        for k in range(1,9):
            ...
            plt.title('PSNR according to k times timestep', fontsize=16)
            ...
            plt.title('SSIM according to k times timestep', fontsize=16)
            plt.xlabel('epochs')
            plt.ylabel('SSIM')
            plt.legend()
        ...
        pc_song = controllable_generation.get_pc_radon_song(sde,
                                                            ...
                                                            radon=radon_all,
                                                            lamb=0.7)
        x = pc_song(score_model, scaler(img), scaler(high), mask, measurement=sinogram_full)
        x_grid = make_grid(x, nrow=1, padding=2, normalize=True)
        ...
        cmh = controllable_generation.get_pc_radon_UNet_plus(sde, predictor, corrector, inverse_scaler, snr=0.209,
                                                            probability_flow=False, continuous=config.training.continuous, mask=mask,
                                                            radon=radon, radon_all=radon_all, save_root=date_root, name=config_name, lamb_schedule=lamb_schedule)
        x = cmh(score_model, data=scaler(img), label=scaler(high), measurement=sinogram_full, jump=2)
        x_grid = make_grid(x, nrow=1, padding=2, normalize=True)
        save_image(x_grid, f'{date_root}/last_image.png')
    elif solver == 'dataset':
