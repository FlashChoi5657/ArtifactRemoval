...
def test(config, workdir='/nas/users/minhyeok/CMH/sde'):
    ...
    date_root = f'{workdir}/result/{time_stamp}/song'
    ...
    test_dl = DataLoader(dataset=test_set, batch_size=config.eval.batch_size, drop_last=True, shuffle=True)
    high, img = next(iter(test_dl))
    
    high_sample = make_grid(high, nrow=int(np.sqrt(config.eval.batch_size)), padding=2, normalize=True)
    save_image(high_sample, f'{date_root}/label_image.png')
    img_sample = make_grid(img, nrow=int(np.sqrt(config.eval.batch_size)), padding=2, normalize=True)
    ...
    if solver == 'mcg':
        ...
        save_image(x_grid, f'{date_root}/MCG/last_image.png')
    elif solver == 'song':
        ...
        x_grid = make_grid(x, nrow=int(np.sqrt(config.eval.batch_size)), padding=2, normalize=True)
        save_image(x_grid, f'{date_root}/song/{config_name}_last_image.png')
    elif solver == 'cmh':
        # corrector = AnnealedLangevinDynamics
        cmh = controllable_generation.get_pc_radon_Artifact(sde, predictor, corrector, inverse_scaler, snr=0.209,
                                                            probability_flow=False, continuous=config.training.continuous, weight=0.1,
                                                            radon=radon_all, save_root=date_root, name=config_name, lamb=0.227)
        x = cmh(score_model, data=scaler(img), label=scaler(high), measurement=sinogram_full)
        x_grid = make_grid(x, nrow=int(np.sqrt(config.eval.batch_size)), padding=2, normalize=True)
        save_image(x_grid, f'{date_root}/cmh/last_image.png')
