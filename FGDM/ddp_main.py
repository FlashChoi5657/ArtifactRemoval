...
class Dataset_FGDM(data.Dataset):
    def __init__(self, csv_file, root_dir, size=None, test=False):
    ...
    def __getitem__(self, index):
        ...
        # 이미지 불러오기 (RGB → 첫 채널 사용)
        npimg = np.load(image_path) # (192, 192)
        if npimg is None:
            raise FileNotFoundError(f"파일을 열 수 없습니다: {image_path}")
        # npimg = np.transpose(npimg, (2, 0, 1))[0]  # R 채널만 사용
        # npimg = npimg.squeeze() # (512, 512)
        ...
        bilateral_random = random.randint(1, 30)

        # Bilateral filter
        nplabs = nplabs.astype(np.float32)
        nplabs = cv2.bilateralFilter(nplabs, 10, bilateral_random, bilateral_random)
        ...
        # Tensor 변환
        npimg = torch.from_numpy(np.expand_dims(npimg.astype(np.float32), 0))
        nplabs = torch.from_numpy(np.expand_dims(nplabs.astype(np.float32), 0))
...
def main(args):
    ...
    dataset = Dataset_FGDM(csv_file="/home/cmh/Workspace/dataset/Shoulder_512.csv", 
                           root_dir="/home/cmh/Workspace/dataset/LDCT/B_Shoulder_192", size=args.image_size)
...
def init_for_distributed(args):
    ...
    dist.init_process_group(backend="nccl", rank=args.global_rank, 
                            world_size=args.world_size, timeout=timedelta(seconds=300))
...
def parse_args_and_config():
    ...
    parser.add_argument('--batch_size', type=int, default=8, help='input batch size per gpu')
